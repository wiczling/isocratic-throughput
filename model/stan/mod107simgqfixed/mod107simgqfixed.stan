...
data {
  array[2] real logP_sim;
  array[2] real pHmpKa_sim;
  array[13] real logCL;
  real fi;
}
...
transformed data{
  ...
  matrix[8, nColumns_sim] kappaStd;
  matrix[3, nColumns_sim] logomega_dStd;
...
generated quantities {
   ...
   array[nAnalytes_sim,nColumns_sim] vector[10] param;
   array[nAnalytes_sim] vector[10] itheta;
   array[nColumns_sim] vector[3] ctheta;
   matrix[nColumns_sim, 3] logomega_d;
   matrix[nAnalytes_sim, 3] eta;
   matrix[nColumns_sim, 8] kappa;
   vector[nColumns_sim] omega_d1; 
   vector[nColumns_sim] omega_d2;
   vector[nColumns_sim] omega_d3;
   matrix[3, nAnalytes_sim] etaStd;
   matrix[nAnalytes_sim,nColumns_sim] dzeta1;
   matrix[nAnalytes_sim,nColumns_sim] dzeta2;
   matrix[nAnalytes_sim,nColumns_sim] dzeta3;
  
  for (i in 1:3)
    for (j in 1:nAnalytes_sim)
      etaStd[i, j] = normal_rng(0, 1);

  for (i in 1:nAnalytes_sim) {
    for (j in 1:nColumns_sim) {
      dzeta1[i, j] = normal_rng(0, 1);
      dzeta2[i, j] = normal_rng(0, 1);
      dzeta3[i, j] = normal_rng(0, 1);
    }
  }
  ...
  
  {
    for (c in 1:13) {
      for (i in 1:2) {
        real logk_new_hat =
          funlogki(
            param[i,c,1],param[i,c,2],
            param[i,c,3],param[i,c,4],
            param[i,c,5],param[i,c,6],
            param[i,c,7],param[i,c,8],
            param[i,c,9],param[i,c,10],
            fi);
        logksim[c,i] = normal_rng(logk_new_hat, sigma);
      }
    }
  }
}
