...
data {
  ...
  int<lower=1> K;
  array[K] real fi_sim_new;
  array[K] int column_sim_new;
}
...
generated quantities {
  array[K] vector[nAnalytes_sim] n_logkobs;
  array[K] real n_fi=fi_sim_new;
  array[K] int  n_column=column_sim_new;
  {
    for (k in 1:K) {
      for (i in 1:nAnalytes_sim) {
        int c = column_sim_new[k];
        real logk_new_hat =
          funlogki(
        ...
        n_logkobs[k,i] = normal_rng(logk_new_hat, sigma);
      }
    }
  }
}